...
class AudioFile:
    """Thin wrapper so BytesIO behaves like st.UploadedFile for our purposes."""
...
#calculating a z-score with a floor so that the percentages returned by hume are consistently weighted in the final ranking
MIN_STD = 0.02  # 2 percentage points minimum spread
def zscore_with_floor(series, min_std=MIN_STD):
    """Z-score a pandas Series with a floor on std dev to prevent
    tiny differences from dominating when all models score similarly."""
    mean = series.mean()
...
with st.sidebar:
    ...
    zip_file = st.file_uploader("Upload a ZIP of audio clips", type=["zip"])
    st.markdown('<div class="info-box" style="font-size:0.78rem">Each file in the ZIP becomes one model. Filename (without extension) is used as the model name.</div>', unsafe_allow_html=True)

    st.markdown("---")
    TARGET_EMOTIONS = [
        "Calmness", "Excitement", "Interest", "Sympathy",
        "Surprise (positive)", "Anger", "Contempt",
        "Disgust", "Distress", "Fear", "Sadness",
    ]
    target_emotion = st.selectbox(
        "🎯 Target Emotion",
        options=TARGET_EMOTIONS,
        help="The emotion the TTS clips are attempting to convey. Used later for Hume scoring.",
    ...
    with zipfile.ZipFile(io.BytesIO(zip_file.read())) as zf:
        for entry in sorted(zf.namelist()):
            ...
            if ext not in (".wav", ".mp3", ".flac"):
                continue
            model_name = os.path.splitext(os.path.basename(entry))[0]
            audio_bytes = zf.read(entry)
            ready_files.append((model_name, AudioFile(os.path.basename(entry), audio_bytes)))
...
comp_matrix = {name: r["components"] for name, r in results.items()}
...
# Use floored z-score for each acoustic feature column
df_zscored = df_norm.apply(zscore_with_floor, axis=0)
acoustic_z = df_zscored.mean(axis=1)

# If arousal available, blend it in
if use_arousal:
    arousal_scores = pd.Series({
        name: (results[name]["arousal"] or {}).get("arousal", 0)
        for name in model_list
    })
    arousal_series = arousal_scores
    arousal_z = zscore_with_floor(arousal_scores)
    composite = 0.7 * acoustic_z + 0.3 * arousal_z
else:
    composite = acoustic_z

# Blend Hume target emotion score into composite if available
hume_raw = pd.Series({
    name: results[name].get("hume_target_score", np.nan)
    for name in model_list
})

#NOTE: the hume scores do not process right away once the site loads. when the rankings page first opens it won't include the hume scores. but after the hume part renders it should update
if hume_raw.notna().all():
    hume_z = zscore_with_floor(hume_raw)
    # Re-weight: acoustic 50%, arousal 30%, hume 20%
    if use_arousal:
        composite = 0.50 * acoustic_z + 0.30 * arousal_z + 0.20 * hume_z
    else:
        composite = 0.80 * acoustic_z + 0.20 * hume_z
...
with tab4:
    ...
    for name, predictions in all_hume_results.items():
        ...
        if all_segment_emotions:
            ...
            # Store target emotion score for later use in rankings
            target_score = global_means.get(target_emotion, 0.0)
            results[name]["hume_target_score"] = target_score
            results[name]["hume_global_means"] = global_means

            st.write("**Top 3 dominant emotions (averaged across utterances):**")
            cols = st.columns(3)
            for i in range(3):
                cols[i].metric(
                    label=global_means.index[i],
                    value=f"{global_means.values[i]:.2%}"
                )

            # Target emotion callout
            st.markdown(f"""<div class="info-box">
                🎯 <strong>Target emotion ({target_emotion}):</strong> 
                raw score = <code>{target_score:.2%}</code>
                &nbsp;—&nbsp; normalized score shown in Rankings tab once all models are processed.
            </div>""", unsafe_allow_html=True)

            # Bar chart — highlight target emotion
            bar_colors = [
                "#cc44ff" if e == target_emotion else colors.get(name, "#33aaff")
                for e in global_means.head(10).index
            ]
            ...
            st.plotly_chart(fig_hume, width='stretch')
